import {
  Carousel,
  CarouselContent,
  CarouselItem,
  CarouselNext,
  CarouselPrevious,
} from "@/components/ui/carousel"
import { Category } from "@/types"
import CategoryCard from "../place/CategoryCard"

export default function CategoryList({ categories }: { categories: Category[] }) {
  return (
    <Carousel className="w-full max-w-275 relative">
      <CarouselContent className="-ml-1">
        {categories.map(category => (
          <CarouselItem key={category.id} className="pl-1 not-first:pl-4 basis-1/2 sm:basis-1/3 md:basis-1/4 lg:basis-1/6">
            <CategoryCard category={category} />
          </CarouselItem>
        ))}
      </CarouselContent>
      <div className="absolute -top-11 right-0 flex items-center gap-2">
        <CarouselPrevious className="static translate-0 border-gray-400" />
        <CarouselNext className="static translate-0  border-gray-400" />
      </div>
    </Carousel>
  )
}
